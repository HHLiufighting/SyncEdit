...
class Denoiser(nn.Module):
    ...
    def forward(
        ...
        **additional_model_inputs,
        ...
        c_in = c_in.to(target_device)
        c_noise = c_noise.to(target_device)
        v = network(input * c_in, c_noise, cond, **additional_model_inputs)
        if v.device != target_device:
            v = v.to(target_device)
        v_u, v_c = v.chunk(2)
        return v_u, v_c, (v * c_out + input * c_skip)
