...
class GeneralConditioner(nn.Module):
        ...
        for embedder in self.embedders:
            ...
            with embedding_context():
                if hasattr(embedder, "input_key") and (embedder.input_key is not None):
                    ...
                    input_data = batch[input_key]

                    # 处理大批量数据（如 21 帧）
                    if (
                            input_key in ["cond_frames", "cond_frames_without_noise"]
                            and isinstance(input_data, torch.Tensor)
                            and input_data.dim() == 4  # [B, C, H, W]
                            and input_data.shape[0] > encode_batch_size
                    ):
                        print(
                            f"Batch encoding {input_key}: {input_data.shape[0]} images with batch size {encode_batch_size}")

                        total_images = input_data.shape[0]
                        all_emb_out = []
                        target_device = input_data.device  # 👈 记录原始设备

                        try:
                            for start_idx in range(0, total_images, encode_batch_size):
                                end_idx = min(start_idx + encode_batch_size, total_images)
                                batch_images = input_data[start_idx:end_idx].contiguous()

                                with torch.cuda.amp.autocast(enabled=True):
                                    emb_out_batch = embedder(batch_images)

                                    if isinstance(emb_out_batch, torch.Tensor):
                                        all_emb_out.append(emb_out_batch.cpu())  # 暂存 CPU 节省显存
                                    else:
                                        all_emb_out.append(emb_out_batch)

                                torch.cuda.empty_cache()

                            # 合并并移回原始设备（关键修复）
                            if isinstance(all_emb_out[0], torch.Tensor):
                                emb_out = torch.cat([x.to(target_device) for x in all_emb_out], dim=0)
                            else:
                                emb_out = all_emb_out
                        ...
                        # 正常处理（小批量或非图像输入）
                        emb_out = embedder(input_data)

                elif hasattr(embedder, "input_keys"):
                    emb_out = embedder(*[batch[k] for k in embedder.input_keys])

                else:
                    raise ValueError(f"Embedder {embedder} has no valid input_key or input_keys.")
                ...
                for emb in emb_out:
                    ...
                    # 强制置零（用于 unconditional conditioning）
                    if (
    ...
    def get_unconditional_conditioning(
        ...
        c = self(batch_c, force_cond_zero_embeddings, encode_batch_size)
        uc = self(batch_c if batch_uc is None else batch_uc, force_uc_zero_embeddings, encode_batch_size)
...
class FrozenOpenCLIPImageEmbedder(AbstractEmbModel):
    ...
    def encode_with_vision_transformer(self, img):
        ...
        # Run resize/normalize in fp32 to avoid unsupported Half reflection-pad paths,
        # then cast back to the visual tower dtype before the actual OpenCLIP forward.
        with torch.autocast("cuda", enabled=False):
            img = self.preprocess(img.to(dtype=torch.float32))
        visual_dtype = next(self.model.visual.parameters()).dtype
        img = img.to(dtype=visual_dtype)
